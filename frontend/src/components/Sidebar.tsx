...
const Sidebar: React.FC = () => {
    ...
    return (
        <div
            className={`bg-sidebar-bg h-full border-r border-border-color flex flex-col transition-all duration-300 ease-in-out relative z-10 ${isCollapsed ? 'w-sidebar-collapsed' : 'w-sidebar-expanded'
                }`}
            id="sidebar"
        >
            <div className="h-header-height flex items-center px-4 border-b border-transparent">
                <button
                    className="bg-transparent border-none cursor-pointer p-2 rounded-full text-text-secondary flex items-center justify-center hover:bg-black/5"
                ...
                <span
                    className={`text-xl font-medium ml-3 whitespace-nowrap text-text-primary transition-opacity duration-300 ${isCollapsed ? 'opacity-0 pointer-events-none hidden' : 'opacity-100'
                        }`}
                >
                    Wisdom Book
                </span>
...
const NavItem: React.FC<NavItemProps> = ({ to, icon, label, active, isCollapsed }) => (
    ...
    >
        ...
        <span
            className={`ml-4 transition-opacity duration-300 ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100'
                }`}
        >
            {label}
        </span>
